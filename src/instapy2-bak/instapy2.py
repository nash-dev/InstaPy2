...
class InstaPy2(InstaPy2Base):
    def comment(self, amount: int = 10, iterable: List[Union[int, str]] = [], type: CommentType = CommentType.Locations, **kwargs):
        _, randomize_media, _, skip_top = parse_args(kwargs=kwargs)

        match type:
            case CommentType.Locations:
                for index, location in enumerate(iterable=iterable):
                    print(f'[{Colors.OKGREEN}INFO{Colors.ENDC}]: comment.locations: location {index + 1} of {len(iterable)}.')
                    ...
                    for index, media in enumerate(iterable=medias):
                        print(f'[{Colors.OKGREEN}INFO{Colors.ENDC}]: comment.locations: media {index + 1} of {len(medias)}.')
            ...
            case _:
                print(f'[{Colors.WARNING}ERROR{Colors.ENDC}]: comment.locations: invalid type.')


    def follow(self, amount: int = 10, iterable: List[Union[int, str]] = [], type: FollowType = FollowType.Commenters, **kwargs):
        _, randomize_media, randomize_tags, skip_top = parse_args(kwargs=kwargs)
    ...
    def like(self, amount: int = 10, iterable: List[Union[int, str]] = [], type: LikeType = LikeType.Feed, **kwargs):
        """
            Possible **kwargs
                randomize_likes, randomize_media, randomize_tags, skip_top
        """

        randomize_likes, randomize_media, randomize_tags, skip_top = parse_args(kwargs=kwargs)
